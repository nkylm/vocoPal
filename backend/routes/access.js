...
router.get('/shared-with', authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user).populate('sharedWith.userId', 'name email role initials avatarColor relation');
...
router.get('/has-access-to', authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user).populate('hasAccessTo.userId', 'name email role initials avatarColor relation');
...
router.post('/share', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const alreadyShared = currentUser.sharedWith.some(p => 
      p.userId && p.userId.toString() === targetUser._id.toString()
    );
    ...
    res.json({ message: 'Permission updated successfully', sharedWith: currentUser.sharedWith });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
...
router.delete('/permissions/:userId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Permission removed successfully', sharedWith: currentUser.sharedWith });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
